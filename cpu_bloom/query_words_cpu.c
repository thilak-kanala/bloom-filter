...
{
    N_WORDS_TO_QUERY = atoi(argv[1]);
    // N_WORDS_TO_INSERT = atoi(argv[2]);
    ...
    prepare_input_data(words_to_query, N_WORDS_TO_QUERY, FILE_WORDS_TO_QUERY);

    // uint32_t *bloom_filter = (uint32_t *)calloc(ceil(BLOOM_FILTER_SIZE / 32.0), sizeof(uint32_t));
    /* Bloom Filter */
    struct BLOOM_FILTER bf;
    read_bloom_filter(&bf, BLOOM_FILTER_FILE);
